...
const fragmentShader = `
  ...
  void main() {
    ...
    // Optional: depth-based dimming (keep if you like it)
    // float depthNorm = clamp(vDepth / maxDepth, 0.0, 1.0);
    // float depthFactor = mix(1.0, 0.7, depthNorm);     
    // baseColor *= depthFactor;

    // ---- Distance fade toward farColor ----
    float fadeT = smoothstep(fadeStart, fadeEnd, vDepth);
    vec3 farBlendColor = mix(baseColor, farColor, fadeT);
